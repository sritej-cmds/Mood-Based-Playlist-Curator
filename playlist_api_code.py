...
"""LOLLLLLLLLLLLL"""
# ───────────────────────────────────────────────
#  RAG (for smart query enhancement using songdesc.csv)
# ───────────────────────────────────────────────
import pandas as pd
from sentence_transformers import SentenceTransformer, util
import numpy as np
...
try:
    ...
    if not os.path.exists(file_path):
        print(f"⚠ songdesc.csv not found at: {file_path}")
    else:
        ...
        print(f"Successfully loaded {len(descriptions)} descriptions for RAG")
...
# ───────────────────────────────────────────────
#  RAG QUERY ENHANCEMENT FUNCTION
# ───────────────────────────────────────────────
def enhance_query_with_rag(query):
    ...
    if not descriptions or desc_embeddings is None:
        print("RAG data not loaded — using raw query.")
        return query
    ...
    try:
        ...
        print("RAG query enhancement error:", e)
        return query
    
def clean_for_spotify(text):
    """
    Keep only 3–5 strong emotion or vibe words for Spotify search.
    """
    import re
    text = text.lower()
...
print("Models loaded successfully: MiniLM + Flan-T5 + DistilBART + Spotipy")
...
# ───────────────────────────────────────────────
# SPOTIFY FETCHER (uses RAG + smart vibe keywords + artist search)
# ───────────────────────────────────────────────
def get_playlist(search, max_results=20):
    ...
    try:
        ...
        if any(w in lower_query for w in sad_triggers) and "sad" not in moods:
            moods.insert(0, "sad")
            print("Sadness trigger detected — added 'sad'")

        # Step ⿢ — Combine moods into clean keywords
        search_terms = ", ".join(moods[:4])  # limit to 4 mood words
        print(f" Using moods for Spotify search → {search_terms}")

        # Step ⿣ — Pick 10 major artists
        top_artists = [
            # English / Global Pop
            ...
            #kannada
            "Raghu dixit", "Chandan Shetty", "Vasuki Vaibhav"
        ]
        random.shuffle(top_artists)
        selected_artists = top_artists[:10]

        # Step ⿤ — Spotify searches using moods + artists
        ...
        print(f" Playlist built with {len(popular_tracks)} tracks (avg popularity ≈ {round(avg_pop, 1)})")
...
def get_similar_song(song_name):
    ...
    try:
        ...
        print("⚠️ Spotify recommendation error:", e)
        return None

"""LOLLLLLLLLL"""
# ───────────────────────────────────────────────
# EMOTION FILTER (semantic)
# ───────────────────────────────────────────────
...
def reflect_on_decision(moods, intent, weather, tod):
    ...
    prompt = (
        ...
        f"Write 3–5 warm, first-person sentences in a {tone} tone. "
    ...
    try:
        ...
        if len(reflection.split()) < 15 or reflection.lower().startswith("the listener"):
            examples = [
                f"I can feel the {mood_text} in your words. I made this {intent} playlist for you — gentle where it needs to be and steady where it should hold you. Sometimes, the right song just says what we can’t.",
                f"I sensed that {mood_text} in you tonight. I built this {intent} playlist to keep you company — not to fix the feeling, but to make space for it.",
        ...
        # small hopeful addition for positive moods
        if any(p in moods for p in {"hopeful", "romantic", "energetic", "happy"}):
            reflection += " " + random.choice([
                ...
                "Even in this feeling, there’s still movement — a rhythm waiting to rise again.",
...
def playlist_with_context():
    ...
    # RAG-enhanced search + normal pipeline
    ...
    playlist = {
        ...
        "time_of_day": tod,
        "weather": weather,
        "context_note": f"{tod.capitalize()} mood detected — matching songs ahead.",
...
# ───────────────────────────────────────────────
# PUT — Add or Update a Song in a Playlist
# ───────────────────────────────────────────────
...
# ───────────────────────────────────────────────
# DELETE — Remove a Song + Suggest Similar
# ───────────────────────────────────────────────
